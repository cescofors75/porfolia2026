    ...
    <div className="grid lg:grid-cols-[.9fr_1.1fr] gap-10 lg:gap-16 items-center">
      ...
      <figure>
        <div className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-border/50 bg-black group">
          <div className={`absolute inset-0 transition-all duration-700 ${slide === 0 ? "opacity-100 scale-100" : "opacity-0 scale-[1.015] pointer-events-none"}`}><Image src="/daisy/daisyRopa.jpeg" alt={media.captions[0]} fill className="object-cover" style={{ objectPosition: "50% 78%" }} sizes="(max-width: 1024px) 100vw, 48vw" /></div>
          <div className={`absolute inset-0 transition-all duration-700 ${slide === 1 ? "opacity-100 scale-100" : "opacity-0 scale-[1.015] pointer-events-none"}`}><Image src="/daisy/daisyPodParther.jpeg" alt={media.captions[1]} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 48vw" /></div>
